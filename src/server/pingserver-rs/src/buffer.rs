...
impl Buffer {
    ...
    pub fn advance_rx(&mut self, bytes: usize) {
        self.rx.advance(bytes);
    }
    ...
    pub fn read_from<T: Read + Debug>(&mut self, source: &mut T) -> Result<usize> {
        trace!("existing data: {} bytes", self.rx.len());
